...
data1 <- read.csv("student-por.csv", sep = ";", stringsAsFactors = T)
summary(data1)
data2 <- data1[,-31]
data2 <- data2[,-31]
summary(data2)
...
round(RMSE.mfull.train,5) #2.29039
round(RMSE.mfull.test,5) #3.06114
...
mfor.1 <- lm(G3 ~ failures + school + higher + Dalc + studytime + schoolsup + health + goout + Fedu + sex + age + famrel + address + romantic, data = trainset.step)
plot(mfor.1)
...
round(RMSE.mfor.train,5) #2.42398
round(RMSE.mfor.test,5) #2.82974
...
for (i in 1:folds){
  ...
  mfold1.train <- lm(G3 ~ failures + school + higher + Dalc + studytime + schoolsup + health + goout + Fedu + sex + age + famrel + address + romantic, data = trainset.step)
...
round(RMSE.fold1.train.mean,5) #2.42398
round(RMSE.fold1.test.mean,5) #2.53305
...
round(RMSE.fold1.test.se, 5) #0.10473
...
mback.1 <- lm(G3 ~ school + sex + age + address + Fedu + studytime + failures + schoolsup + higher + romantic + famrel + goout + Dalc + health, data = trainset.step)
plot(mback.1)
...
round(RMSE.mback.train,5) #2.42398
round(RMSE.mback.test,5) #2.82974
...
for (i in 1:folds){
  ...
  mfold2.train <- lm(G3 ~ school + sex + age + address + Fedu + studytime + failures + schoolsup + higher + romantic + famrel + goout + Dalc + health, data = trainset.step)
...
round(RMSE.fold2.train.mean,5) #2.42398
round(RMSE.fold2.test.mean,5) #2.53305
...
round(RMSE.fold2.test.se,5) #0.10473
